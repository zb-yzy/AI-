...
SAVE_DIR = "../../results/results1/All_Trans_results"
...
if __name__ == "__main__":
    ...
    # ===================== 修改模型：ResNet18 + Transformer注意力 =====================
    model = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
    # 插入Transformer注意力
    model.trans_attn = TransformerAttention(in_channels=512, num_heads=8)
    # 替换分类头
    model.fc = nn.Linear(model.fc.in_features, NUM_CLASSES)

    # 重写前向传播，在 layer4 之后、avgpool 之前加入注意力
    def new_forward(x):
        x = model.conv1(x)
        x = model.bn1(x)
        x = model.relu(x)
