...
SAVE_NAME = "Emotic_Trans_balanced"
...
IMG_SIZE = 224
BATCH_SIZE = 32          # ResNet18 显存占用小，可恢复原 batch size
LR = 1e-4
EPOCHS = 15
...
class TransformerAttention(nn.Module):
    def __init__(self, in_channels, num_heads=8, dropout=0.1):
        ...
        # 适配 224输入 -> layer4输出 7*7=49 序列长度
        self.pos_embed = nn.Parameter(torch.zeros(1, 49, in_channels))
...
# ===================== 构建 ResNet18 + TransformerAttention 回归模型 =====================
def create_resnet18_with_transformer_regression(output_dim=3, pretrained=True):
    if pretrained:
        model = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
    else:
        model = models.resnet18(weights=None)

    # 插入 Transformer 注意力模块
    model.trans_attn = TransformerAttention(in_channels=512, num_heads=8, dropout=0.1)
    # 替换分类头为回归头
    ...
    def new_forward(x):
        ...
        x = model.layer2(x)
        x = model.layer3(x)
        x = model.layer4(x)          # 输出形状: (B, 512, 7, 7)
        x = model.trans_attn(x)      # 自定义 Transformer 注意力
        x = model.avgpool(x)
        x = torch.flatten(x, 1)
        x = model.fc(x)
        return x
...
if __name__ == "__main__":
    ...
    # 5. 模型、损失函数、优化器
    model = create_resnet18_with_transformer_regression(output_dim=3, pretrained=True)
