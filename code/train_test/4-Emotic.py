...
# ===================== 用户配置 =====================
DATA_ROOT = "../../data/Emotic"   # 包含 face/ 和 csv/ 的根目录
FACE_DIR = os.path.join(DATA_ROOT, "face")      # 存放人脸图片的文件夹
CSV_DIR = os.path.join(DATA_ROOT, "csv")        # 存放标注CSV文件的文件夹

SAVE_NAME = "Emotic_VAD"
...
EXAMPLES_SAVE_DIR = os.path.join(SAVE_DIR, "test_examples")   # 保存样例的文件夹
...
class EmoticVADDataset(Dataset):
    def __init__(self, df, img_dir, transform=None):
        ...
        for idx, row in df.iterrows():
            ...
            labels = torch.tensor(labels_list[:3], dtype=torch.float32)
...
def save_regression_examples(model, loader, device, save_dir, max_save=50, horizontal=True):
    """
    保存测试集中预测误差最小（好）和最大（差）的图片。
    在图片旁边扩展一个区域显示标注信息，不覆盖原图。
    
    Args:
        horizontal: True -> 右侧扩展（水平拼接），False -> 下方扩展（垂直拼接）
    """
    ...
    all_info = []
    with torch.no_grad():
        for imgs, labels in tqdm(loader, desc="收集测试样本"):
            imgs = imgs.to(device)
            preds = model(imgs)
    ...
    for idx, (img_tensor, true_lab, pred_lab, err) in enumerate(good_samples):
        ...
        # 准备文本内容
        lines = [
            f"True VAD: ({true_lab[0]:.2f}, {true_lab[1]:.2f}, {true_lab[2]:.2f})",
        ...
        for line in lines:
            ...
            text_height += line_h + 5  # 行间距
        text_height += 10  # 上下边距

        if horizontal:
            # 水平拼接：左侧原图，右侧白色文本区域
            new_width = img_pil.width + text_width + 40   # 右侧额外留白
        ...
        print(f"保存好例子 {idx+1}/{len(good_samples)}: {save_path}")
    ...
    for idx, (img_tensor, true_lab, pred_lab, err) in enumerate(bad_samples):
        ...
        new_img.save(save_path)
        print(f"保存差例子 {idx+1}/{len(bad_samples)}: {save_path}")
...
if __name__ == "__main__":
    ...
    for csv_file in csv_files:
        ...
        for col in required_cols:
            if col not in df.columns:
                raise ValueError(f"文件 {csv_file} 缺少列 {col}，实际列名: {df.columns.tolist()}")
    ...
    # 2. 划分训练集、验证集、测试集（70%训练，15%验证，15%测试）
    ...
    train_dataset = EmoticVADDataset(train_df, FACE_DIR, transform=train_transform)
    val_dataset = EmoticVADDataset(val_df, FACE_DIR, transform=val_test_transform)
    test_dataset = EmoticVADDataset(test_df, FACE_DIR, transform=val_test_transform)
    ...
    # 测试集
    test_loss, test_mae, test_rmse, test_mae_per_dim, test_rmse_per_dim, y_true_test, y_pred_test = evaluate(model, test_loader, criterion, DEVICE)
    ...
    with open(os.path.join(SAVE_DIR, "final_results.txt"), 'w') as f:
        ...
        f.write(f"RMSE: {val_rmse:.6f}\n")
        f.write(f"维度MAE: {val_mae_per_dim}\n")
        f.write(f"维度RMSE: {val_rmse_per_dim}\n\n")
        f.write("========== 测试集结果 ==========\n")
        f.write(f"MAE: {test_mae:.6f}\n")
        f.write(f"RMSE: {test_rmse:.6f}\n")
        f.write(f"维度MAE: {test_mae_per_dim}\n")
        f.write(f"维度RMSE: {test_rmse_per_dim}\n")
