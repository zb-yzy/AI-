...
SAVE_DIR = "../../results/results2/Movie_Balance_results"
...
# 自定义平铺数据集：从文件名解析标签 xxx_序号_label.jpg
...
if __name__ == "__main__":
    ...
    # 训练循环
    print(f"\n🚀 训练开始 | 平铺无随机平衡数据集 | 从 epoch {start_epoch + 1} 训练 {CONTINUE_EPOCHS} 轮 | 设备：{DEVICE}\n")
    ...
    # 绘制三张子图曲线（损失、总体acc、balanced acc）
    ...
    plt.subplot(1, 3, 3)
    plt.plot(balanced_accs, label="测试平均准确率 (每类准确率均值)", color='green')
    plt.title("平均准确率曲线")
    plt.legend()
    plt.grid()
